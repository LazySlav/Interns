...
from web.models import WorkersTable
...
def main(request: HttpRequest, id: int | None = None):
    ...
    if request.method == 'GET':
        ...
        __id_check(id)
        try:
            obj = WorkersTable.objects.get(id=id)
        except (WorkersTable.DoesNotExist, WorkersTable.MultipleObjectsReturned):
            raise
        ...
        try:
            obj = WorkersTable.objects.get(id=id)
        except (WorkersTable.DoesNotExist, WorkersTable.MultipleObjectsReturned) as e:
            raise
        data = __parse_request(request)
        data.pop("id")
